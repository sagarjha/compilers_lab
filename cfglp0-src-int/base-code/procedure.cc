...
bool Procedure::match_argument_list(list<argument*> *arg_list) {
    if (function_defined) {
	report_error("The function has already been defined",NOLINE);
    }
...
Eval_Result & Procedure::evaluate(ostream & file_buffer)
{
    ...
    // push a variable called return in the eval_env
  
    file_buffer << "\n\n\n";
    file_buffer << LOC_VAR_SPACE << "Local Variables (after evaluating) Function: << " << name << " >>\n";
    eval_env.print_with_return(file_buffer, true);
    ...
    eval_env.print_with_return(file_buffer, false);
