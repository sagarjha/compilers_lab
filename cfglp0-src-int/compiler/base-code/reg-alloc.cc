...
void Lra_Outcome::optimize_lra(Lra_Scenario lcase, Ast * destination_memory, Ast * source_memory)
{
	...
	case mc_2m:
	    ...
	    if ((typeid(*source_memory) == typeid(Number_Ast<int>)) || (typeid(*source_memory) == typeid(Expr_Ast)))
		source_register = NULL;
	    else
		{
	    ...
	    else if (destination_register != NULL)
		{
			...
			result_register = machine_dscr_object.get_new_register(source_memory->get_data_type());
			load_needed = true;
		    }
...
void Machine_Description::initialize_instruction_table()
{
    ...
    spim_instruction_table[bne] = new Instruction_Descriptor(bne, "bne", "bne", "            ", i_op_o1_o2_r, a_op_o1_o2_r);
    spim_instruction_table[goto_command] = new Instruction_Descriptor(goto_command, "goto", "j", "           ", i_op_o1, a_op_o1);
    ...
    spim_instruction_table[div_i] = new Instruction_Descriptor(div_i, "div", "div", "            ", i_r_o1_op_o2, a_op_r_o1_o2);
...
Register_Descriptor * Machine_Description::get_new_register(Data_Type data_type)
{
    Register_Val_Type type = float_num;
